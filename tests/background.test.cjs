...
test('two conditions already satisfied at initial connection produce distinct stage alerts',async()=>{const h=harness();h.setNow('2026-09-10');await h.connect();assert.deepEqual(h.notifications.map(n=>n.n),['trip-first','trip-reminder']);});
...
test('unsupported holiday year keeps deadline notifications',async()=>{const h=harness();h.setNow('2028-09-08');h.setRecords([rec('b','접수대기',true,'2028-09-14')]);await h.connect();assert.equal(h.notifications.length,2);assert.equal(h.data.local.status.calendarWarning,false);});
...
test('all alert categories show only their matching student names',async()=>{const h=harness();h.setNow('2026-09-11');h.setRecords([{...rec('a'),studentName:'테스트가'}]);h.setDepartures([{key:'d'.repeat(64),startDate:'2026-09-14',studentName:'테스트나'}]);await h.connect();for(const kind of ['first','reminder','departure']){const item=h.data.local.alertLog.find(i=>i.kind===kind);assert.ok(item);assert.ok(item.message.includes(kind==='departure'?'테스트나':'테스트가'));assert.ok(!item.message.includes(kind==='departure'?'테스트가':'테스트나'));}});
