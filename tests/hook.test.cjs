...
function harness(){
 ...
 function arm(trusted=true){events['neis-trip-arm-v2']({detail:{nonce:'test-nonce'}});events.click({isTrusted:trusted,target:{closest:()=>({textContent:'조회'})}});}
 ...
 return {arm,xhr,posted,window,reinject:()=>vm.runInContext(code,context)};
}
...
test('repeated injection does not stack fetch or XHR capture hooks',async()=>{const h=harness(),fetch=h.window.fetch;h.reinject();h.reinject();assert.equal(h.window.fetch,fetch);h.arm();h.xhr();await h.window.fetch('/observed.do',{method:'POST',body:'{}'});await new Promise(r=>setImmediate(r));assert.equal(h.posted.length,2);assert.ok(h.posted.every(p=>p.source==='NEIS_TRIP_CAPTURE_V2'));});
