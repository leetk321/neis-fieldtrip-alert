(() => {
  ...
  function showNextNotice(){
    ...
    Object.assign(panel.style,{position:'fixed',right:'24px',bottom:'64px',zIndex:'2147483647',background:next.kind==='departure'?'#643da5':'#123c46',color:'white',padding:'16px 20px',borderRadius:'12px',maxWidth:'410px',maxHeight:'60vh',overflowY:'auto',whiteSpace:'pre-line',font:'14px/1.8 sans-serif',boxShadow:'0 8px 30px #0003'});
  ...
  function validateScreen(config) {
    const heading=Array.from(document.querySelectorAll('[role="heading"],h1,h2,h3,.cl-text')).some(e=>visible(e)&&e.textContent.trim()==='교외체험학습신청서관리');
    if(!heading || !grid()) throw Error('교외체험학습신청서관리 화면을 열어 주세요.');
    const read=name=> {
    ...
    if(read('접수상태')!=='전체'||read('결재상태')!=='전체')throw Error('접수상태와 결재상태를 모두 전체로 설정하세요.');
  ...
  async function normalized(data,schema,config) {
    let learnedSchema;
    if(schema.pending){
      const resolved=C.resolvePending(data,schema);
      if(resolved.pending)return {records:[],departures:[],count:0,total:0,awaitingSchema:true,needsConfirmation:resolved.needsConfirmation};
  ...
  window.addEventListener('message',async event=> {
    ...
    try {
      ...
      const schema=total===0?C.pendingSchema(data,total):C.learn(data,total), endpoint=C.endpoint(d.url,location.origin);
      ...
      const snapshot=await normalized(data,schema,pending.config);
      const reply=await send({type:'CAPTURE',nonce:pending.nonce,identity:who,template:{endpoint,body:d.body,headers:d.headers,schema},snapshot});
    ...
    }catch(error){notice(error.message);await send({type:'CAPTURE_ERROR',error:error.message}).catch(()=>{});}
  ...
  chrome.runtime.onMessage.addListener((message,sender,reply)=> {
    ...
    (async()=> {
      if(message.type==='ARM') {
        validateScreen(message.config);
        const who=await identity();
        pending={nonce:message.nonce,config:message.config,identity:who,until:Date.now()+60000};
      ...
      if(message.type==='POLL') {
        ...
        try {
          ...
          try {
            ...
            return {ok:true,snapshot:await normalized(data,t.schema,message.config)};
